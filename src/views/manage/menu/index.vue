...
import { fetchDeleteMenuById, fetchDeleteMenuByIds, fetchGetMenuList, fetchGetMenus } from '@/service/api';
...
const { columns, columnChecks, data, loading, pagination, getData, getDataByPage } = useTable({
  ...
  columns: () => [
    ...
    },
    {
      ...
      width: 80,
      render: row => {
        const tagMap: Record<Api.MenuManage.MenuType, NaiveUI.ThemeColor> = {
          1: 'default',
          2: 'primary'
        };
...
const editingData: Ref<Api.MenuManage.Menu | null> = ref(null);

function handleEdit(item: Api.MenuManage.Menu) {
  operateType.value = 'edit';
  editingData.value = { ...item };
  openModal();
}

function handleAddChildMenu(item: Api.MenuManage.Menu) {
...
async function getAllPages() {
  const { data: pages } = await fetchGetMenus();
