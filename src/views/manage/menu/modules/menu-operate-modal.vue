...
interface Props {
  ...
  rowData?: Api.MenuManage.Menu | null;
  /** all pages */
  allPages: string[];
}
...
type Model = Pick<
  ...
  | 'fixedIndexInTab'
> & {
  query: NonNullable<Api.MenuManage.Menu['query']>;
  buttons: NonNullable<Api.MenuManage.Menu['buttons']>;
  layout: string;
  page: string;
  pathParam: string;
};
...
function handleCreateButton() {
  const buttonItem: Api.MenuManage.MenuButton = {
    code: '',
    desc: ''
  };
...
async function handleSubmit() {
  ...
  if (props.operateType === 'edit') {
    ...
    const { error } = await fetchInsertMenu(params);
